## Code credit: https://github.com/xiangyue9607/CliniRC/blob/master/src/preprocessing.py
...
parser.add_argument('--out_dir', type=str, help='Path to output file dir', default='./emrQA')
...
def main(args):
    ...
    subset_name = ['medication', 'relation', 'risk-dataset']
...
def clean_datasets(filename):
    ...
    for i in range(len(data["data"])):
        ...
        if actual_data['title'] not in ["medication", "relations", "risk-dataset"]:
            continue
        new_data = dict()
        ...
        for j in tqdm(range(len(actual_data["paragraphs"]))):
            ...
            for k in range(len(qas)):
                ...
                for l in range(len(answers)):
                    if "answer_entity_type" in answers[l] and answers[l]["answer_entity_type"] == 'complex':
                        continue
                    new_answers = dict()
                    if actual_data['title'] == 'risk-dataset':
